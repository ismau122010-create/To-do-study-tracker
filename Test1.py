...
def delete(): # create the delete function
     ...
     if user == "delete all":   # this will delte evertyhing in the data list
        ...
        user_delete_task = input("Enter task to be deleleted: ")
        To_Do_data[:] = [
...
def study_session():
    ...
    user_subject_duration = int(input("Enter duration: "))
     
    if not user_subject.isalpha():        # if the user input for subk=ject doesnt have string return the following error. 
        raise ValueError("Error: task must contain letters only (no numbers, spaces, or symbols)")

    found = False

    for session in study_session_data:          # loop through the list and if the same task exist , only update the duration by adding 
     if session["subject"].lower() == user_subject.lower():                          # the new duration. 
        session["duration"] += user_subject_duration
        found = True
        break

     if not found:                                             # if the task not found , add new one subject. 
